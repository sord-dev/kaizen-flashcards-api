...
class Card {
  ...
  async changeContent(data) {
    ...
    const resp = await db.query("UPDATE cards SET question = $1,description = $2, answer = $3 WHERE card_id = $4 RETURNING *;", [question, description, answer, this.card_id])
  ...
  async Destroy() {
    try {
      const resp = await db.query("DELETE FROM cards WHERE card_id = $1 RETURNING card_id", [this.card_id]);
