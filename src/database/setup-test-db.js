...
const createDbEnv = async () => {
    ...
    await db.query("CREATE TEMPORARY TABLE user_stats (LIKE user_stats INCLUDING ALL)")
}

const populateDbEnv = async () => {
    await db.query("INSERT INTO user_stats (amount, correct) VALUES (0, 0);")
    await db.query(`INSERT INTO pg_temp.users (username, password, user_stats_id, streak) VALUES ('admin', '$2b$10$/QzbJnjbKX7Bgma7L9DfGu3wRMKFMfCtc.Jekyh3PbxDoSjuPEngm', 1, '{ "count": 1,"lastHit": "2023-04-22T07:24:16.165Z", "startDate": "2023-04-22T07:24:16.165Z"}');`)
...
const destroyDbEnv = async () => {
    ...
    await db.query("DROP TABLE IF EXISTS pg_temp.token")
    await db.query("DROP TABLE IF EXISTS pg_temp.user_stats")
